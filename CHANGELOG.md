...
- 清理死代码与未使用导入；
- **修复命令不被识别**：全部命令 pattern 放宽尾随空白（`\s*$`），解决手机
  输入法尾随空格导致指令被当成普通聊天的问题；
- **修复 `/mai lxns qq`**：未绑定 QQ 时给出「未绑定落雪玩家」友好提示
  （接口本身实测可用）；
- **修复落雪社区别名导入为零**：公共别名接口真实结构为
  `{song_id, aliases: []}`（单次全量返回、无分页），按真实结构解析，
  实测一次导入 2672 条；
- **修复收藏品/头像/封面图片**：资源 CDN URL 增加 `!webp` 后缀绕过 WAF
  挑战页（实测可下载真实 webp，收藏品实物图/头像/落雪封面兜底恢复）。
